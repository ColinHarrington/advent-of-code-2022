...
fn starting_items(input: &str) -> IResult<&str, Vec<u64>> {
    preceded(tag("  Starting items: "), separated_list1(tag(", "), nom_u64))(input)
}
...
fn operation(input: &str) -> IResult<&str, Operation> {
    let (input, (operator, operand)) = preceded(
        ...
        separated_pair(operator, nom_char(' '), operand))(input)?;

    Ok((input, Operation { operator, operand }))
}

fn test_expression(input: &str) -> IResult<&str, TestExpression> {
    let (input, divisible_by) = preceded(tag("  Test: divisible by "), terminated(nom_u64, line_ending))(input)?;
    let (input, if_true) = preceded(tag("    If true: throw to monkey "), terminated(nom_u64, line_ending))(input)?;
    let (input, if_false) = preceded(tag("    If false: throw to monkey "), nom_u64)(input)?;

    Ok((input, TestExpression { divisible_by, if_true, if_false }))
}

fn monkey(input: &str) -> IResult<&str, Monkey> {
    let (input, number) = preceded(tag("Monkey "),
                                   terminated(nom_u64,
                                              terminated(nom_char(':'),
                                                         line_ending)))(input)?;
    ...
    Ok((input, Monkey { number: number as usize, starting_items, operation, test }))
}
...
Monkey 3:
  ...
  Test: divisible by 17
    ...
    fn parsing_starting_items() {
        assert_eq!(Ok(("", vec![79, 98])), starting_items("  Starting items: 79, 98"));
        assert_eq!(Ok(("", vec![54, 65, 75, 74])), starting_items("  Starting items: 54, 65, 75, 74"));
        assert_eq!(Ok(("", vec![79, 60, 97])), starting_items("  Starting items: 79, 60, 97"));
    ...
    If true: throw to monkey 2
    If false: throw to monkey 3";
        assert_eq!(Ok(("", TestExpression { divisible_by: 23, if_true: 2, if_false: 3 })), test_expression(monkey0));

        let monkey1 = r"  Test: divisible by 19
    If true: throw to monkey 2
    If false: throw to monkey 0";
        assert_eq!(Ok(("", TestExpression { divisible_by: 19, if_true: 2, if_false: 0 })), test_expression(monkey1));

        let monkey2 = r"  Test: divisible by 13
    If true: throw to monkey 1
    If false: throw to monkey 3";
        assert_eq!(Ok(("", TestExpression { divisible_by: 13, if_true: 1, if_false: 3 })), test_expression(monkey2));

        let monkey3 = r"  Test: divisible by 17
    If true: throw to monkey 0
    If false: throw to monkey 1";
        assert_eq!(Ok(("", TestExpression { divisible_by: 17, if_true: 0, if_false: 1 })), test_expression(monkey3));
    }

    #[test]
    fn parsing_operation() {
        assert_eq!(Ok(("", Operation { operator: Operator::Times, operand: Operand::Value(19) })), operation("  Operation: new = old * 19"));
        assert_eq!(Ok(("", Operation { operator: Operator::Plus, operand: Operand::Value(6) })), operation("  Operation: new = old + 6"));
        assert_eq!(Ok(("", Operation { operator: Operator::Times, operand: Operand::Old })), operation("  Operation: new = old * old"));
    }
